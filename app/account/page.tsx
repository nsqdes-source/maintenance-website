    ...
    <section className="requestTableWrap">
      ...
      {!requests?.length ? <div className="emptyState"><h2>لا توجد طلبات</h2><p>يمكنك إرسال أول طلب خدمة من نموذج طلب الخدمة.</p></div> : (
        <div className="requestList">{requests.map((request) => {
          ...
          return <article className="requestAdminCard" key={request.id}>
            ...
            <div className="requestAdminDetails">
              ...
              <div><strong>رقم الجوال</strong><span>{request.phone}</span></div>
              <div><strong>موقع الخدمة</strong>{hasCoordinates ? <><span dir="ltr">{request.latitude.toFixed(6)}, {request.longitude.toFixed(6)}</span><a href={mapUrl!} target="_blank" rel="noreferrer" className="requestMapLink">فتح الموقع في خرائط Google</a></> : <span>غير محدد</span>}</div>
            </div>
          </article>;
        })}</div>
      )}
