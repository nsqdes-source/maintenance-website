...
export default async function AccountPage() {
  ...
  const [{ data: profile, error: profileError }, { data: requests, error: requestsError }] =
    await Promise.all([
      supabase
        .from("profiles")
      ...
      <main className="adminPage">
        <div className="container adminContainer">
          <div className="form-error" role="alert">
            تعذر تحميل بيانات الحساب حاليًا. حاول مرة أخرى لاحقًا.
          </div>
        </div>
      </main>
    );
    ...
    <main className="adminPage">
      <div className="container adminContainer">
        <div className="adminTopbar">
          ...
          <div>
            <a className="button secondary" href="/request">طلب خدمة جديدة</a>
            <AccountSignOut />
          </div>
        </div>
        ...
        <div className="emptyState">
          ...
          {profile?.phone && <p>رقم الجوال: {profile.phone}</p>}
        </div>

        <section className="requestTableWrap">
          <div className="requestTableHeader">
            <span>{requests?.length ?? 0} طلب</span>
            <span>طلباتك فقط</span>
          </div>

          {!requests?.length ? (
            <div className="emptyState">
              <h2>لا توجد طلبات</h2>
              <p>يمكنك إرسال أول طلب خدمة من نموذج طلب الخدمة.</p>
            </div>
          ) : (
                ...
                <article className="requestAdminCard" key={request.id}>
                  <div className="requestAdminMain">
                    ...
                    <p className="requestMeta">
                      {request.city} · {new Date(request.created_at).toLocaleString("ar-SA")}
                    </p>
