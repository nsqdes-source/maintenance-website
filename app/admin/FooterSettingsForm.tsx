...
export default function FooterSettingsForm({
  ...
  function update(
    field: Exclude<
      keyof FooterContent,
      "payment_methods" | "payment_logo_urls" | "social_links"
    >,
    value: string
  ) {
  ...
  function togglePayment(id: string) {
    setForm((current) => ({
      ...current,
      payment_methods: current.payment_methods.includes(id)
        ? current.payment_methods.filter((item) => item !== id)
  ...
  value: string | boolean | number
    ...
    setForm((current) => ({
      ...current,
      social_links: current.social_links.map((item) =>
        item.id === id
          ? {
              ...item,
              [field]: value,
    ...
    <section className="card footerSettingsCard">
      <div className="sectionHeading">
        ...
        <p className="sectionIntro">
          حدّث البيانات التي تظهر للزوار في أسفل الموقع دون تعديل الكود.
        </p>
      </div>
      ...
      <div className="formGrid">
        ...
        <label className="fullField">
          ...
          <input
            ...
            onChange={(e) =>
              update("copyright_text", e.target.value)
            }
          />
        </label>
      </div>
      ...
      <div className="footerExtras">
        <div>
          <p className="eyebrow">مركز الأعمال</p>
          <h3>رابط وشعار مركز الأعمال</h3>
          <p>
            يظهر للزوار في التذييل، ويمكن ربطه ببوابة الشركاء أو الموظفين.
          </p>
        </div>
        ...
        <div className="formGrid">
          <label>
            نص الرابط
            <input
              value={form.business_center_label}
              onChange={(e) =>
                update("business_center_label", e.target.value)
              }
            />
          </label>
          ...
          <label>
            ...
            <input
              dir="ltr"
              value={form.business_center_url}
              onChange={(e) =>
                update("business_center_url", e.target.value)
              }
            />
          </label>
          ...
          <div className="fullField">
            <ImageUploadField
              label="شعار مركز الأعمال"
              value={form.business_center_logo_url || null}
              onChange={(url) =>
                update("business_center_logo_url", url || "")
              }
            />
          </div>
        </div>
      ...
      <div className="footerExtras">
        <div>
          <p className="eyebrow">مواقع التواصل الاجتماعي</p>
          <h3>روابط وأيقونات المنصات</h3>
          <p>
            أضف رابط الحساب ورابط صورة الأيقونة. إذا تركت رابط الحساب فارغًا فلن تظهر المنصة في التذييل.
          </p>
        </div>

        <div className="socialOptionGrid">
          {form.social_links.map((social) => (
            <div
              key={social.id}
      ...
      <div className="footerExtras">
        <div>
          <p className="eyebrow">طرق السداد</p>
          <h3>الأيقونات الظاهرة للزوار</h3>
          <p>
            اختر فقط الوسائل التي تتوفر لديك فعليًا.
          </p>
        </div>

        <div className="paymentOptionGrid">
          {PAYMENT_OPTIONS.map((option) => (
            ...
            >
              ...
              <input
                className="paymentLogoUrl"
                dir="ltr"
                type="url"
                placeholder="https://.../logo.png"
                value={form.payment_logo_urls[option.id] || ""}
                onChange={(event) =>
                  updatePaymentLogo(
                    option.id,
                    event.target.value
                ...
                aria-label={`رابط شعار ${option.label}`}
              />
            </div>
          ))}
        </div>
