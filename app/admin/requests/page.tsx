...
type SearchParams = { status?: string; from?: string; to?: string; archive?: string; sort?: string };
...
export default async function AdminRequestsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const archive = filters.archive === "yes";
  const sort = filters.sort === "oldest" ? "oldest" : "newest";
  const to = filters.to && /^\d{4}-\d{2}-\d{2}$/.test(filters.to) ? filters.to : "";

  let requestsQuery = supabase
    .from("service_requests")
    .select("id, customer_name, phone, service_type, problem_description, city, address, workflow_stage, visit_notes, created_at, archived_at")
    .order("created_at", { ascending: sort === "oldest" });

  requestsQuery = archive ? requestsQuery.not("archived_at", "is", null) : requestsQuery.is("archived_at", null);
  ...
  for (const row of stageRows ?? []) stageCounts.set(row.workflow_stage, (stageCounts.get(row.workflow_stage) ?? 0) + 1);
  ...
  return <main className="adminPage"><div className="container adminContainer">
    ...
    <form className="card" method="get" aria-label="تصفية الطلبات">
      <div className="requestAdminDetails">
        ...
        <div><label htmlFor="archive">العرض</label><select id="archive" name="archive" defaultValue={archive ? "yes" : "no"}><option value="no">الطلبات الحالية</option><option value="yes">الأرشيف</option></select></div>
        <div><label htmlFor="sort">الترتيب</label><select id="sort" name="sort" defaultValue={sort}><option value="newest">الأحدث</option><option value="oldest">الأقدم</option></select></div>
      </div>
