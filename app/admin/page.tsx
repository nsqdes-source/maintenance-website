...
export default async function AdminPage() {
  ...
  const initialFooter = { company_name: footerContent?.company_name ?? "معين لخدمات الصيانة", description: footerContent?.description ?? "معين.. الصيانة أسهل", phone: footerContent?.phone ?? "", email: footerContent?.email ?? "", address: footerContent?.address ?? "", copyright_text: footerContent?.copyright_text ?? "© 2026 جميع الحقوق محفوظة", business_center_label: footerContent?.business_center_label ?? "مركز الأعمال", business_center_url: footerContent?.business_center_url ?? "/admin/login", business_center_logo_url: footerContent?.business_center_logo_url ?? "", payment_methods: Array.isArray(footerContent?.payment_methods) ? footerContent.payment_methods as string[] : ["mada", "visa", "mastercard", "apple_pay", "bank_transfer"], payment_logo_urls: footerContent?.payment_logo_urls && typeof footerContent.payment_logo_urls === "object" && !Array.isArray(footerContent.payment_logo_urls) ? footerContent.payment_logo_urls as Record<string, string> : {} };
