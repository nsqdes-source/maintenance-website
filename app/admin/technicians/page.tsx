...
export default async function AdminTechniciansPage() {
  ...
  const [{ data: technicians, error }, { data: technicianLinks }, { data: profiles, error: profilesError }, { data: assignments }] = await Promise.all([
    ...
    supabase.from("service_request_assignments").select("technician_id, status"),
  ]);
  ...
  for (const assignment of assignments ?? []) {
    if (["pending", "accepted"].includes(assignment.status)) {
      requestCounts.set(assignment.technician_id, (requestCounts.get(assignment.technician_id) ?? 0) + 1);
    }
  }
      ...
      {error ? <div className="form-error">تعذر تحميل الفنيين.</div> : !technicians?.length ? (
        ...
        <div className="adminTableWrap">
          <table className="adminTable">
            <thead><tr><th>الفني</th><th>الجوال</th><th>الخدمات</th><th>الحالة</th><th>الطلبات المسندة</th><th>الملاحظات</th><th>تاريخ الإضافة</th><th>الإجراء</th></tr></thead>
            <tbody>{technicians.map((technician) => {
              ...
              return <tr key={technician.id}>
                ...
                <td>{requestCounts.get(technician.id) ?? 0}</td>
