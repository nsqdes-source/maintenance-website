...
export default async function AdminTechniciansPage() {
  ...
  return <main className="adminPage"><div className="container adminContainer">
    ...
    {error ? <div className="form-error">تعذر تحميل الفنيين.</div> : !technicians?.length ? <div className="emptyState"><h2>لا يوجد فنيون حتى الآن</h2><p>أضف فنيًا من مستخدم مسجل، وستظهر بياناته هنا.</p></div> : (
      <div className="technicianCardGrid">{technicians.map((technician) => { const p = Array.isArray(technician.profile) ? technician.profile[0] : technician.profile; const name = p?.full_name || "فني بدون اسم"; const activeAssignments = assignmentCounts.get(technician.id) ?? 0; return <article className="technicianCard" key={technician.id}>
        <div className="technicianCardHeader"><div className="technicianAvatar" aria-hidden="true">{name.trim().charAt(0) || "ف"}</div><div className="technicianCardIdentity"><div className="technicianNameRow"><h2>{name}</h2><span className={`statusBadge ${technician.is_active ? "status-active" : "status-inactive"}`}>{technician.is_active ? "نشط" : "غير نشط"}</span></div><a href={`/admin/technicians/${technician.id}`} className="technicianDetailsLink">عرض تفاصيل الفني ←</a></div></div>
        <div className="technicianCardMeta"><div><span>الجوال</span><strong>{p?.phone || "—"}</strong></div><div><span>الطلبات النشطة</span><strong>{activeAssignments}</strong></div></div>
        <div className="technicianServices">{technician.service_types?.length ? technician.service_types.map((service: string) => <span key={service}>{service}</span>) : <span>لا توجد خدمات محددة</span>}</div>
        {technician.notes ? <p className="technicianNotes">{technician.notes}</p> : null}
        <div className="technicianCardActions"><a className="button secondary compactButton" href={`/admin/technicians/${technician.id}`}>التفاصيل</a><TechnicianEditControl technicianId={technician.id} initialServices={technician.service_types ?? []} initialActive={technician.is_active} initialNotes={technician.notes} /></div>
      </article>; })}</div>
    )}
    <style>{`.technicianCardGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.technicianCard{padding:22px;border:1px solid #e2e8f0;border-radius:18px;background:#fff;box-shadow:0 8px 24px rgba(15,23,42,.04)}.technicianCardHeader{display:flex;align-items:flex-start;gap:14px}.technicianAvatar{width:48px;height:48px;flex:0 0 48px;display:grid;place-items:center;border-radius:14px;background:#0f172a;color:#fff;font-size:1.15rem;font-weight:800}.technicianCardIdentity{min-width:0;flex:1}.technicianNameRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.technicianNameRow h2{margin:0;font-size:1.1rem}.technicianDetailsLink{display:inline-block;margin-top:7px;color:#475569;font-size:.85rem;font-weight:700}.technicianCardMeta{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:20px}.technicianCardMeta div{padding:12px;border-radius:12px;background:#f8fafc}.technicianCardMeta span,.detailFacts span,.assignmentCardMeta span{display:block;color:#64748b;font-size:.78rem}.technicianCardMeta strong{display:block;margin-top:4px}.technicianServices{display:flex;flex-wrap:wrap;gap:7px;margin-top:14px}.technicianServices span{padding:5px 9px;border:1px solid #e2e8f0;border-radius:999px;color:#475569;font-size:.78rem}.technicianNotes{margin:14px 0 0;color:#64748b;line-height:1.7;font-size:.9rem}.technicianCardActions{display:flex;align-items:center;gap:8px;margin-top:18px}.compactButton{min-height:40px;padding-inline:14px;font-size:.85rem}.technicianCardActions .technicianEditControl{flex:1}.technicianCardActions .technicianEditControl>button{width:100%}@media(max-width:900px){.technicianCardGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.technicianCardGrid{grid-template-columns:1fr}}`}</style>
  </div></main>;
}
