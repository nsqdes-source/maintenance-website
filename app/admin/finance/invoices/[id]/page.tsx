...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: invoice }, { data: lines }] = await Promise.all([
    ...
    supabase.from("invoice_line_items").select("description,quantity,unit_price,sort_order").eq("invoice_id", id).order("sort_order"),
  ]);
  if (!invoice) notFound();
  ...
  return <main className="adminPage"><div className="container adminContainer">
    ...
    <article className="invoiceSheet" dir="rtl"><header><p className="eyebrow">{invoice.status === "draft" ? "مسودة غير صادرة" : invoice.status === "void" ? "ملغاة" : invoice.vat_registered ? "مسودة ضريبية غير صالحة للإصدار" : "مستند غير ضريبي"}</p><h1>فاتورة #{invoice.invoice_number}</h1><p>تاريخ {new Date(invoice.issued_at || invoice.created_at).toLocaleDateString("ar-SA")}</p></header>
      ...
      <table className="financeTable"><thead><tr><th>البيان</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th></tr></thead><tbody>{lines?.length ? lines.map((line,index) => <tr key={index}><td>{line.description}</td><td>{line.quantity}</td><td>{money(line.unit_price)}</td><td>{money(Number(line.quantity) * Number(line.unit_price))}</td></tr>) : <tr><td>{invoice.description}</td><td>1</td><td>{money(invoice.subtotal)}</td><td>{money(invoice.subtotal)}</td></tr>}<tr><td colSpan={3}>الضريبة ({invoice.tax_rate}%)</td><td>{money(invoice.tax_amount)}</td></tr><tr><th colSpan={3}>الإجمالي</th><th>{money(invoice.total)}</th></tr></tbody></table>
      {invoice.status === "draft" ? <p className="formMessage">مسودة للمراجعة؛ لا ترسلها للعميل قبل إصدارها.</p> : null}
    </article>
    <InvoiceDraftEditor invoiceId={invoice.id} status={invoice.status} initialWorkSummary={invoice.work_summary || invoice.description} initialLines={(lines ?? []).map(line => ({ description: line.description, quantity: Number(line.quantity), unit_price: Number(line.unit_price) }))} />
  </div></main>;
}
