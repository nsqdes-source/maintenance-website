...
import styles from "./FinanceSettingsClient.module.css";
...
export default function FinanceSettingsClient({
  ...
  async function disconnectDrive() {
    if (!window.confirm("فصل حساب Google Drive عن الموقع؟")) {
      return;
    }
    ...
    if (!response.ok) {
      setMessage(result.error || "تعذر فصل حساب Google Drive.");
      return;
    }
  ...
  return (
    <>
      ...
      <section className="card financePanel">
        ...
        <p>تُستخدم هذه البيانات في الفواتير والمستندات المالية.</p>
      ...
      <section className="card financePanel">
        ...
        <p>حدد حالة التسجيل الضريبي قبل إصدار الفواتير.</p>
