  ...
  return (
    <div ref={menuRef} style={{ display: "flex", alignItems: "center", gap: 10, position: "relative", flex: "0 0 auto" }}>
      ...
      {open && <div role="menu" style={{ position: "absolute", top: "calc(100% + 10px)", left: 0, width: "min(256px, calc(100vw - 32px))", padding: 12, border: "1px solid #dbe3ee", borderRadius: 14, background: "#fff", boxShadow: "0 16px 42px rgba(15,23,42,.18)", zIndex: 1000, direction: "rtl" }}>
        <div style={{ padding: "3px 5px 10px", borderBottom: "1px solid #e2e8f0" }}><strong style={{ display: "block", marginBottom: 8, fontSize: ".95rem", color: "#0f172a" }}>بيانات الحساب</strong><div style={{ display: "grid", gap: 6, color: "#475569", fontSize: ".82rem", lineHeight: 1.55 }}><span>الاسم: {profile?.full_name || "—"}</span><span style={{ overflowWrap: "anywhere" }}>البريد الإلكتروني: {email || "—"}</span><span>رقم الجوال: {profile?.phone || "—"}</span><span>نوع الحساب: {ROLE_LABELS[profile?.role ?? ""] ?? profile?.role ?? "—"}</span></div></div>
        <a href="/account/edit" role="menuitem" aria-label="تعديل المعلومات" title="تعديل المعلومات" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, marginTop: 9, padding: "9px 10px", border: "1px solid #e2e8f0", borderRadius: 8, background: "#fff", color: "#0f172a", fontSize: ".82rem", fontWeight: 700, textDecoration: "none" }}><svg viewBox="0 0 24 24" aria-hidden="true" style={{ width: 17, height: 17, fill: "none", stroke: "currentColor", strokeWidth: 1.8, strokeLinecap: "round", strokeLinejoin: "round" }}><path d="M12 20h9" /><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L8 18l-4 1 1-4Z" /></svg><span>تعديل المعلومات</span></a>
