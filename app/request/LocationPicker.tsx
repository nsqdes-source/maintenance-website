...
export default function LocationPicker({ value, onChange }: LocationPickerProps) {
  const mapElement = useRef<HTMLDivElement | null>(null); const mapRef = useRef<any>(null); const markerRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      const googleMaps = (window as any).google.maps; const center = value ? { lat: value.latitude, lng: value.longitude } : defaultCenter;
      ...
      mapRef.current.addListener("click", (event: any) => { if (!event.latLng) return; const position = { lat: event.latLng.lat(), lng: event.latLng.lng() }; if (markerRef.current) markerRef.current.setPosition(position); else markerRef.current = new googleMaps.Marker({ position, map: mapRef.current }); onChange({ latitude: position.lat, longitude: position.lng }); setError(""); });
      setLoading(false);
    };
    ...
    if (existingScript) { existingScript.addEventListener("load", initializeMap); return () => { cancelled = true; existingScript.removeEventListener("load", initializeMap); }; }
    const script = document.createElement("script"); script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(apiKey)}&v=weekly`; script.async = true; script.defer = true; script.dataset.googleMaps = "true"; script.onload = initializeMap; script.onerror = () => { if (!cancelled) { setError("تعذر تحميل Google Maps."); setLoading(false); } }; document.head.appendChild(script);
