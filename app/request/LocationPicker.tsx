...
export default function LocationPicker({ value, onChange }: LocationPickerProps) {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      setError("لم يتم إعداد مفتاح Google Maps في بيئة الموقع.");
      setLoading(false);
      return;
    }
    ...
    const initializeMap = () => {
      ...
      mapRef.current = new googleMaps.Map(mapElement.current, {
        center,
        zoom: value ? 15 : 11,
      ...
      if (value) {
        markerRef.current = new googleMaps.Marker({ position: center, map: mapRef.current });
      }
    ...
    if (existingScript) {
      ...
      existingScript.addEventListener("error", () => {
        if (!cancelled) {
          setError("تعذر تحميل Google Maps.");
          setLoading(false);
        }
      });
      return () => {
      ...
      ({ coords }) => {
        ...
        if (mapRef.current) {
          ...
          if (markerRef.current) {
            ...
            const googleMaps = (window as any).google.maps;
            markerRef.current = new googleMaps.Marker({ position, map: mapRef.current });
          }
        }
      },
      () => setError("تعذر تحديد موقعك الحالي. يمكنك اختيار الموقع يدويًا على الخريطة."),
  ...
  return (
    <div className="location-picker">
      <div className="location-picker-actions">
        <button type="button" className="button secondary" onClick={useCurrentLocation} disabled={loading}>
