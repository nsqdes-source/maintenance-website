...
export default function RequestForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (userId && (!profile.full_name?.trim() || !phonePattern.test(profile.phone ?? "") || !emailPattern.test(email))) {
      setStatus({ success: false, message: "بيانات حسابك غير مكتملة أو غير صحيحة. حدّث بيانات الحساب أولًا ثم أعد إرسال الطلب." }); setPending(false); return;
    }
    if (!phonePattern.test(phone)) { setStatus({ success: false, message: "رقم الجوال يجب أن يتكون من 10 أرقام ويبدأ بـ 0." }); setPending(false); return; }
    if (!emailPattern.test(email)) { setStatus({ success: false, message: "يرجى إدخال بريد إلكتروني صحيح." }); setPending(false); return; }
    if (!cities.includes(city)) { setStatus({ success: false, message: "يرجى اختيار مدينة صحيحة من القائمة." }); setPending(false); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="request-form">
      {!userId && !loadingUser && <>
        <div className="form-group"><label htmlFor="customer_name">الاسم *</label><input id="customer_name" name="customer_name" type="text" required placeholder="اكتب اسمك" autoComplete="name" /></div>
        <div className="form-group"><label htmlFor="phone">رقم الجوال *</label><input id="phone" name="phone" type="tel" required placeholder="05xxxxxxxx" autoComplete="tel" dir="ltr" inputMode="numeric" maxLength={10} pattern="0[0-9]{9}" title="يجب أن يتكون رقم الجوال من 10 أرقام ويبدأ بـ 0" /></div>
        <div className="form-group"><label htmlFor="customer_email">البريد الإلكتروني *</label><input id="customer_email" name="customer_email" type="email" required placeholder="name@example.com" autoComplete="email" /></div>
      </>}

      {userId && !loadingUser && <fieldset className="form-group" disabled={pending}><legend>بيانات العميل</legend>
        <div className="form-group"><label htmlFor="account_customer_name">الاسم</label><input id="account_customer_name" type="text" value={profile.full_name ?? ""} readOnly aria-readonly="true" /></div>
        <div className="form-group"><label htmlFor="account_phone">رقم الجوال</label><input id="account_phone" type="tel" value={profile.phone ?? ""} readOnly aria-readonly="true" dir="ltr" /></div>
        <div className="form-group"><label htmlFor="account_email">البريد الإلكتروني</label><input id="account_email" type="email" value={userEmail} readOnly aria-readonly="true" dir="ltr" /></div>
        {!profile.full_name || !phonePattern.test(profile.phone ?? "") || !emailPattern.test(userEmail) ? <p className="form-hint">بيانات الحساب غير مكتملة. حدّث بيانات الحساب أولًا قبل إرسال الطلب.</p> : <p className="form-hint">هذه البيانات مأخوذة من حسابك ولا يمكن تعديلها داخل طلب الخدمة.</p>}
      </fieldset>}

      <div className="form-group"><label htmlFor="service_type">نوع الخدمة *</label><select id="service_type" name="service_type" required defaultValue=""><option value="">اختر نوع الخدمة</option><option value="الكهرباء">الكهرباء</option><option value="التكييف">التكييف</option><option value="السباكة">السباكة</option><option value="النجارة">النجارة</option><option value="خدمات أخرى">خدمات أخرى</option></select></div>
