...
begin
  ...
  if current_stage in ('completed','cancelled','customer_rejected','customer_cancelled') then raise exception 'request_already_closed'; end if;
  if current_stage = 'technician_accepted' then raise exception 'technician_already_accepted'; end if;
  update public.service_requests
  set workflow_stage = 'customer_cancelled', status = 'cancelled',
      visit_notes = case
