-- Protect active technician assignments from role/status changes.
-- A technician with pending/accepted assignments cannot be deactivated or converted
-- to a non-technician role until those assignments are no longer active.
...
REVOKE ALL ON FUNCTION public.admin_update_user_role(uuid, public.app_role, public.app_role) FROM PUBLIC;
